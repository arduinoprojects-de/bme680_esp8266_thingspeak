...
WiFiClient  client;
unsigned long thingspeak_prev = millis();
...
void output_ready(int64_t timestamp, float iaq, uint8_t iaq_accuracy, float temperature, float humidity,
     float pressure, float raw_temperature, float raw_humidity, float gas, float co2_equivalent, float breath_voc_equivalent, bsec_library_return_t bsec_status)
{
    Serial.print("[");
    ...
    Serial.print("| BreathVOCeq: ");
    Serial.println(breath_voc_equivalent);

    if(millis() - thingspeak_prev > 60000){  
         thingspeak_postData(iaq,iaq_accuracy,temperature,humidity,pressure,gas,co2_equivalent,breath_voc_equivalent); 
         thingspeak_prev = millis();
    }
}

void thingspeak_postData(float iaq, uint8_t iaq_accuracy, float temperature, float humidity,
     float pressure, float gas, float co2_eq, float bvoc_eq) {
  ...
  ThingSpeak.setField(3, float(pressure / 100.0));
  ThingSpeak.setField(4, iaq);
  ThingSpeak.setField(5, gas);
  ThingSpeak.setField(6, iaq_accuracy);
  ThingSpeak.setField(7, co2_eq);
  ThingSpeak.setField(8, bvoc_eq);
  
  int x = ThingSpeak.writeFields(CONFIG_THINGSPEAK_CHANNEL_ID, CONFIG_THINGSPEAK_API_KEY);
